	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='idtahunajaran') {
			alert('Tahun Ajaran Sebelumnya belum dipilih!');				
		  }
		  
		  if (document.getElementById(arrf[i]).name=='idtahunajaran1') {
			alert('Tahun Ajaran Tujuan belum dipilih!');				
		  }
		  ...
		  if (document.getElementById(arrf[i]).name=='idminat') {
			alert('Peminatan belum dipilih!');				
		  }
		  
		  if (document.getElementById(arrf[i]).name=='idkelas2') {
...
<div class="page-content">
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="client_cheque" id="client_cheque" enctype="multipart/form-data" onSubmit="return cekinput('idtahunajaran,idtahunajaran1,semester_id,idminat,idkelas2');" >
            	
            	<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Tahun Angkatan Sebelumnya *)</label>
					<div class="col-sm-4">
						<select name="idangkatan1" id="idangkatan1" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_angkatan_non_default($idangkatan1); ?>
						</select>								
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Tahun Angkatan Tujuan *)</label>
					<div class="col-sm-4">
						<select name="idangkatan2" id="idangkatan2" class="chosen-select form-control" >
							<?php select_angkatan_default($idangkatan2); ?>
						</select>								
					</div>
				</div>
				<div class="form-group">
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        <input type="submit" name="submit" id="submit" class='btn btn-primary' value="Generate Copy KRS" />
					</div>
				</div>
		...
		<?php
			if($_POST["submit"]) {
				...
				##aktifkan siswa yg belum aktif
				$strsql = "select siswa.replid from siswa left join kelas on siswa.idkelas=kelas.replid where siswa.idkelas='$idkelas2' and kelas.idtingkat=27 and ifnull(siswa.aktif,0)=0 order by siswa.replid";
				$sql=$dbpdo->query($strsql);
				while($data_siswa=$sql->fetch(PDO::FETCH_OBJ)) {
					$strsql = "update `siswa` set aktif=1 where replid='$data_siswa->replid'";
					$sqlx=$dbpdo->prepare($strsql);
					$sqlx->execute();
				}
				##----------/\-----------------
							
				
				if($idangkatan1 != "" && $idangkatan2 != "" && $semester_id != "" && $idminat != "") {
					$sqlkrs = "select a.nis, a.nama, c.replid pegawai_id from `siswa` a left join kelas b on a.idkelas=b.replid left join pegawai c on b.nipwali=c.nip where a.idminat='$idminat' and a.idangkatan1='$idangkatan2' and a.idkelas='$idkelas2' order by a.replid ";
					...
					while ($row_krs = $resultskrs->fetch(PDO::FETCH_OBJ)){
						...
						$nis			= 	$row_krs->nis;
						$nama			= 	$row_krs->nama;
						$pegawai_id		=	$row_krs->pegawai_id;
						
						if($nis != "") {
							
							$no++;
							
							echo 'No.'.$no.'|NIS: '.$nis.'|Nama: '.$nama.'<br>';
							
							##get thn ajaran sblmnya
							$sql="select replid, tahunajaran from tahunajaran where aktif=1 and ifnull(info2,0)=0 order by replid desc limit 1";
							$results=$dbpdo->query($sql);
							$data_ta = $results->fetch(PDO::FETCH_OBJ);
							$old_idtahunajaran = $data_ta->replid;
							
							##get tahn ajaran skg
							$sql="select replid, tahunajaran from tahunajaran where aktif=1 and ifnull(info2,0)=1 order by tahunajaran";
							$results=$dbpdo->query($sql);
							$data_ta = $results->fetch(PDO::FETCH_OBJ);
							$idtahunajaran1 = $data_ta->replid;
							
							##get siswa_krs tahun ajaran yg lalu
							$sqlstr = "select a.nis, a.idtahunajaran, a.semester_id, a.pelajaran_id, a.kelompok_pelajaran_id, a.nilai, a.sks_id, a.pegawai_id, a.approved, a.uid, a.dlu, a.ip_proxy, a.ip_pc, a.mobile, a.browser from siswa_krs a left join pegawai b on a.pegawai_id=b.replid left join kelas c on b.nip=c.nipwali where a.idtahunajaran='$old_idtahunajaran' and a.semester_id='$semester_id' and c.idtingkat=27 order by a.replid";
							$sqlx=$dbpdo->prepare($sqlstr);
							$sqlx->execute();
							while($data_krs = $sqlx->fetch(PDO::FETCH_OBJ)) {
								
								$pelajaran_id			= 	$data_krs->pelajaran_id;
								$kelompok_pelajaran_id	= 	$data_krs->kelompok_pelajaran_id;
								$nilai					=	0;
								$sks_id					=	$data_krs->sks_id;
								$approved				=	1;
								$ip_proxy				=	'';
								$ip_pc					=	'';
								$mobile					=	0;
								...
								$sqlcek = "select a.replid from siswa_krs a where a.nis='$nis' and a.idtahunajaran='$idtahunajaran1' and a.semester_id='$semester_id' and a.pelajaran_id='$pelajaran_id' limit 1 ";
								$resultscek=$dbpdo->query($sqlcek);	
								$rowscek=$resultscek->rowCount();
								if($rowscek == 0) {
									$uid		=	'generate'; //$_SESSION["loginname"];
									$dlu		=	date("Y-m-d H:i:s");
									
									$sqlstr = "insert into siswa_krs (nis, idtahunajaran, semester_id, pelajaran_id, kelompok_pelajaran_id, nilai, sks_id, pegawai_id, approved, uid, dlu, ip_proxy, ip_pc, mobile, browser) values ('$nis', '$idtahunajaran1', '$semester_id', '$pelajaran_id', '$kelompok_pelajaran_id', '$nilai', '$sks_id', '$pegawai_id', '$approved', '$uid', '$dlu', '$ip_proxy', '$ip_pc', '$mobile', '$browser')";
									$sqly=$dbpdo->prepare($sqlstr);
									$sqly->execute();
								}
							}
