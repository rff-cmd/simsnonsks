...
<script>
	...
	function print_raport(ref) {
		var idsiswa		=	document.getElementById('idsiswa').value;
		
		window.open('<?php echo $__folder ?>app/raport_siswa_print.php?idsiswa='+idsiswa, 'Invoice Print','825','450','resizable=1,scrollbars=1,status=0,toolbar=0')
	}
	
	function print_raport_absensi(ref) {
		var idsiswa		=	document.getElementById('idsiswa').value;
		
		window.open('<?php echo $__folder ?>app/raport_siswa_absensi_print.php?idsiswa='+idsiswa, 'Invoice Print','825','450','resizable=1,scrollbars=1,status=0,toolbar=0')
	}
	
	function print_raport_aspek(ref) {
		var idsiswa		=	document.getElementById('idsiswa').value;
		
		window.open('<?php echo $__folder ?>app/raport_siswa_aspek_print.php?idsiswa='+idsiswa, 'Invoice Print','825','450','resizable=1,scrollbars=1,status=0,toolbar=0')
	}
</script>
...
<div class="page-content">
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<form class="form-horizontal" role="form" action="" method="post" name="soal_ukbm_view" id="soal_ukbm_view" enctype="multipart/form-data" >
				...
				<div class="form-group">
                    ...
                    <div class="col-sm-3">
                      <input type="submit" name="submit" id="submit" class='btn btn-primary' value="Tampilkan Raport"/>
                    </div>  
				</div>
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				include("app/exec/insert_raport_siswa.php");
				
				$tanggal = date("d-m-Y");
				if ($ref != "") {
					$sql=$select->list_raport_siswa($ref);
					$row_soal_siswa=$sql->fetch(PDO::FETCH_OBJ);	
					$idsiswa = $row_soal_siswa->idsiswa;
					$idsemester = $row_soal_siswa->idsemester;
					$idukbm = $row_soal_siswa->idukbm;
					$tanggal = date("d-m-Y", strtotime($row_soal_siswa->tanggal));
				}		
			?>
            
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="soal_siswa" id="soal_siswa" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('idsiswa,idsemester');" >
            	...
            	<?php  
            		include("raport_siswa_detail.php");
            	?>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowadd('frmraport_siswa')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Simpan" onClick="return confirm('Apakah sudah yakin akan menyimpan raport ini?')" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php /*if (allowdel('frmsoal_siswa')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="Data Raport" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix(raport_siswa_view) ?>'" />
