...
$bussines_type= "Pendiidkan"; //$row_company->businiss_type;
...
while($row_receipt_detail=$sql2->fetch(PDO::FETCH_OBJ)) {
	...
	if ($invoice_type == "cashier") {
		$invoice_type = "Simpanan Wajib";
	}
	if($invoice_type == "iuran_siswa") {
		$invoice_type = "Iuran Siswa "." ".$row_receipt_detail->description;
	}
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Image('../assets/img/logo.jpg', 10, 5, 25, 25, 'jpg', '');
		$this->Ln(1);
		...
		$this->Cell(26,3,'',0,0,'L',false);
		$this->Cell(50,3,'Alamat :' . $address . ' Telp: ' . $phone,0,1,'L',false);
		$this->Ln(2);
		...
		$this->Cell(145,5,'PENERIMAAN IURAN SISWA',0,0,'L',true);
...
$title='PENERIMAAN SIMPANAN WAJIB';
$pdf->SetTitle($title);	
// $pdf->SetTitle($nis);	
// $pdf->SetTitle($nama);
...
$header=array('No.','No. Invoice','Tanggal','Transaksi','Jumlah','Dibayar');
