  ...
  <Table>
   ...
   <Cell ss:StyleID="judul"><Data ss:Type="String">'.$idpelajaran.'</Data></Cell>
	...
	for($n=1; $n<=$jumlah_ukbm; $n++) {	
		...
		echo '<Cell ss:StyleID="kepala"><Data ss:Type="String">N-'.$n.'(T)</Data></Cell>';
	}
	
	echo '<Cell ss:StyleID="kepala"><Data ss:Type="String">Kehadiran(%)</Data></Cell>';
	...
	while($siswa_view=$sql->fetch(PDO::FETCH_OBJ)){
		...
		$sqlnilai 	= $select->list_daftarnilai_pts($siswa_view->replid, $idtingkat, $idkelas, $idtahunajaran, $semester_id, $idpelajaran, "");
		$siswa_get  = "";
		...
		//get UAS
		/*$sqlnilai_uas 	= $select->list_daftarnilai_pts($siswa_view->replid, $idtingkat, $idkelas, "", $semester_id, $idpelajaran, 3);
		$data_get_uas	= $sqlnilai_uas->fetch(PDO::FETCH_OBJ);
		$uas_p 			= $data_get_uas->uas;
		
		$sqlnilai_uas 	= $select->list_daftarnilai_pts($siswa_view->replid, $idtingkat, $idkelas, "", $semester_id, $idpelajaran, 4);
		$data_get_uas	= $sqlnilai_uas->fetch(PDO::FETCH_OBJ);
		...
		/*if($siswa_get->rata == 0 || $siswa_get->rata == "") {
			...
			$hadir = 0;
			$sqlndetail = $select->list_daftarnilai_pts_detail($siswa_get->replid); //$siswa_view
			$jumlah_ukbm_upd = 0; //$sqlndetail->rowCount();
			while($datanilai_detail=$sqlndetail->fetch(PDO::FETCH_OBJ)) {
				
				$sqldetnilai = $select->list_daftarnilai_detail_dasarpenilaian_pts($siswa_get->departemen, $siswa_get->idtingkat, $siswa_get->idkelas, $siswa_get->idtahunajaran, $siswa_get->idsemester, $siswa_get->nis, $siswa_get->idkompetensi, $siswa_get->idjeniskompetensi, 3, $siswa_get->idpelajaran, $datanilai_detail->line);
				$datadetnilai = $sqldetnilai->fetch(PDO::FETCH_OBJ);
				//$uas_p = $datadetnilai->uas;
				
				if($hadir < $datadetnilai->hadir) {
					$hadir = $datadetnilai->hadir;
				}
				
				echo "<Cell ss:StyleID=\"badan\"><Data ss:Type=\"String\">".$datadetnilai->nilai."</Data></Cell>";
				
				$sqldetnilai1 = $select->list_daftarnilai_detail_dasarpenilaian_pts($siswa_get->departemen, $siswa_get->idtingkat, $siswa_get->idkelas, $siswa_get->idtahunajaran, $siswa_get->idsemester, $siswa_get->nis, $siswa_get->idkompetensi, $siswa_get->idjeniskompetensi, 4, $siswa_get->idpelajaran, $datanilai_detail->line);
				$datadetnilai1 = $sqldetnilai1->fetch(PDO::FETCH_OBJ);
				//$uas_k = $datadetnilai1->uas;
				
				if($hadir < $datadetnilai1->hadir) {
					$hadir = $datadetnilai1->hadir;
				}
				
				echo "<Cell ss:StyleID=\"badan\"><Data ss:Type=\"String\">".$datadetnilai1->nilai."</Data></Cell>";
				
				$sqldetnilai2 = $select->list_daftarnilai_detail_dasarpenilaian_pts($siswa_get->departemen, $siswa_get->idtingkat, $siswa_get->idkelas, $siswa_get->idtahunajaran, $siswa_get->idsemester, $siswa_get->nis, $siswa_get->idkompetensi, $siswa_get->idjeniskompetensi, 7, $siswa_get->idpelajaran, $datanilai_detail->line);
				$datadetnilai2 = $sqldetnilai2->fetch(PDO::FETCH_OBJ);
				//$uas_k = $datadetnilai1->uas;
				
				if($hadir < $datadetnilai2->hadir) {
					$hadir = $datadetnilai2->hadir;
				}
				
				echo "<Cell ss:StyleID=\"badan\"><Data ss:Type=\"String\">".$datadetnilai2->nilai."</Data></Cell>";
				
				
			}
			
			/*for($b=$jumlah_ukbm_upd+3; $b<=$jumlah_ukbm; $b++) {	
				echo "<Cell ss:StyleID=\"badan\"><Data ss:Type=\"String\">".""."</Data></Cell>";
				echo "<Cell ss:StyleID=\"badan\"><Data ss:Type=\"String\">".""."</Data></Cell>";
			}*/
		//}
		echo "<Cell ss:StyleID=\"badan\"><Data ss:Type=\"String\">".$hadir."</Data></Cell>";
