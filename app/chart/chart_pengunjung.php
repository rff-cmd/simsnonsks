...
$sqlstr="select a.* from (select '1' kode, 'Januari' name union all
			  ...
			  select '12' kode, 'Desember' name) a order by a.kode
			";
$sql1 = $dbpdo->query($sqlstr);
while ($row_tingkat=$sql1->fetch(PDO::FETCH_OBJ)) {
	$sqlstr2 = "select count(a.replid) count_surat, month(a.tglpinjam), a.tglpinjam from pinjam a  group by month(a.tglpinjam) having month(a.tglpinjam)='$row_tingkat->kode'";
