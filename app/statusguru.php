	...
	function cekinput(fid) {  
	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='status2') {
		...
		<div class="col-xs-12">
            ...
            <?php 
				...
				if ($ref != "") {
					...
					$row_statusguru=$sql->fetch(PDO::FETCH_OBJ);	
					
					if ($row_statusguru->aktif == 1) {
						$act = "checked";
					} else {
						$act = "";
					}
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="statusguru" id="statusguru" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('status2');" >
            	
            	<input type="hidden" id="id" name="id" value="<?php echo $ref ?>" >
            	    	
            	<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Status Guru *)</label>
					<div class="col-sm-3">
						<input type="text" id="status2" name="status2" class="form-control" value="<?php echo $row_statusguru->status ?>">
					</div>
				</div>
                ...
                <div class="form-group">
					...
					<div class="col-sm-3">
						<textarea id="keterangan" name="keterangan" class="form-control"><?php echo $row_statusguru->keterangan ?></textarea>
					</div>
				</div>
