...
$tanggal_ttd	=	$datasiswa->tanggal_ttd; //$datasemester->tanggal_ttd;
...
$idkelas_wali	=	$datasiswa->idkelas;
...
while($row_pelajaran_detail=$sql->fetch(PDO::FETCH_OBJ)) { 
	...
	$data2[]='Kelompok A (Umum)'.'|'.'Pengetahuan'.'|'.$kode_pelajaran.'|'.$nama_pelajaran.'|'.$deskripsi_p.'|'.$jumlah_nilai;
	$data3[]='Kelompok A (Umum)'.'|'.'Keterampilan'.'|'.$kode_pelajaran.'|'.$nama_pelajaran.'|'.$deskripsi_k.'|'.$jumlah_nilai;
...
while($row_pelajaran_detail=$sql->fetch(PDO::FETCH_OBJ)) { 
	...
	$data2[]='Kelompok B (Umum)'.'|'.'Pengetahuan'.'|'.$kode_pelajaran.'|'.$nama_pelajaran.'|'.$deskripsi_p.'|'.$jumlah_nilai;
	$data3[]='Kelompok B (Umum)'.'|'.'Keterampilan'.'|'.$kode_pelajaran.'|'.$nama_pelajaran.'|'.$deskripsi_k.'|'.$jumlah_nilai;
...
while($row_pelajaran_detail=$sql->fetch(PDO::FETCH_OBJ)) { 
	...
	if (preg_match("/64/",$idpelajaran_check) == 0) {
		...
		$data2[]='Kelompok C (Peminatan)'.'|'.'Pengetahuan'.'|'.$kode_pelajaran.'|'.$nama_pelajaran.'|'.$deskripsi_p.'|'.$jumlah_nilai;
		$data3[]='Kelompok C (Peminatan)'.'|'.'Keterampilan'.'|'.$kode_pelajaran.'|'.$nama_pelajaran.'|'.$deskripsi_k.'|'.$jumlah_nilai;
...
while($row_pelajaran_detail=$sql->fetch(PDO::FETCH_OBJ)) { 
	...
	$data2[]='Kelompok C (Pilihan Lintas Peminatan)'.'|'.'Pengetahuan'.'|'.$kode_pelajaran.'|'.$nama_pelajaran.'|'.$deskripsi_p.'|'.$jumlah_nilai;
	$data3[]='Kelompok C (Pilihan Lintas Peminatan)'.'|'.'Keterampilan'.'|'.$kode_pelajaran.'|'.$nama_pelajaran.'|'.$deskripsi_k.'|'.$jumlah_nilai;
...
class PDF extends FPDF
{
	...
	function BasicTable($header,$data)
	{
		...
		foreach($header as $col) {
			...
			if ($i==3) { $this->Cell(25,7,"Aspek",1,0,"C"); }
			if ($i==4) { $this->Cell(91,7,"Deskripsi",1,0,"C"); }
			$i++;
		}
		$this->Ln();
		...
		for($z=0; $z<count($data_kelompok); $z++) {
			...
			for($s=0; $s<count($data2); $s++) {
				...
				if($exdata[0] == $data_kelompok[$z]) {
					if($exdata[5] > 0) {
						$k++;
						$this->Cell(15,20,$exdata[2]."0".$k.$numeric_semester,1,0,"C"); //kode
