...
while($datasiswa = $sqlsiswa->fetch(PDO::FETCH_OBJ)) {
	...
	while($row_pelajaran_detail=$sql->fetch(PDO::FETCH_OBJ)) { 
		...
		while($datanilai_detail=$sqlndetail->fetch(PDO::FETCH_OBJ)) {
			...
			/*echo '$datanilai_detail->nilai :'. $datanilai_detail->nilai ."<br>";
			echo '$nilai_tertinggi :'. $nilai_tertinggi ."<br>";*/
		}
	...
	$no_urut_mapel = 0;
	$sql = $selectview->list_daftarnilai_raport("SMA", $idtingkat, $idkelas, $idsemester, $idsiswa, 4, $peminatan);
	while($row_pelajaran_detail=$sql->fetch(PDO::FETCH_OBJ)) { 
...
class PDF extends FPDF
{
	...
	function BasicTable($header,$data)
	{
		...
		for($i=0; $i<count($nis_siswa); $i++) {		
			...
			$this->Cell(15,7,$jumlah_sks[$i],1,0,'C',true);
			$this->Cell(13,7,$total_p[$i],1,0,'C',true);
			$this->Cell(13,7,'',1,0,'C',true);
			$this->Cell(13,7,$total_k[$i],1,0,'C',true);
			$this->Cell(13,7,'',1,0,'C',true);
			$this->Cell(13,7,$total_rata[$i],1,0,'C',true);
			$this->Cell(22,7,$total_rata_beban[$i],1,0,'C',true);
			$this->Ln(10);
			$total_rows = $total_rows - 10; //for seting rows
			
			$this->Cell(80,7,'IP Semester ((Rata2 Nilai x Beban)/Jml. Beban)',1,0,'C',true);
			$this->Cell(10,7,$ip_semester[$i],1,0,'C',true);
