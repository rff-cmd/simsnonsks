    ...
    if ($_FILES["file"]["size"] > 0) {
        ...
        while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
        	...
        	if($x > 0) {
        		...
        		$nip =  str_replace(" ","",$column[1]); //$data->val($i, 2);
				...
				if (!empty($nip))	 {
					...
					$cekitem="select nip from pegawai where nip='$nip'";
					$qdata=$conn->query($cekitem);
					$rowsitem=mysqli_num_rows($qdata);
					
					if($rowsitem == 0) {
						$query = "insert into pegawai (nip, nama, bagian, aktif) values ('$nip', '$nama', '$bagian', '$aktif')";	   
