	...
	if(isset($_POST['queryString'])) {
		...
		if(strlen($queryString) >0) {	
			...
			if (user_admin()==0) {
				...
				$query=mysql_query("select distinct a.acccde, a.accdcr from coa a left join coa_dtl b on a.acccde=b.acccde where ifnull(a.ptb,0)=1 and a.act=1 and (a.acccde like '$queryString%' or a.accdcr like '%$queryString%') order by a.acccde ");
			...
			echo '<ul>';
				while ($result = mysql_fetch_object($query) ) {
					echo '<li onClick="fill_'.$no.'(\''.addslashes($result->accdcr).'\'); fill2_'.$no.'(\''.addslashes($result->acccde).'\'); ">'.$result->acccde.'&nbsp;&nbsp;'.$result->accdcr. '</li>';
				}
			echo '</ul>';
